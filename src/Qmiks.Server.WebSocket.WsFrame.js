 ...
 * @version:0.91.008 websocket 数据侦类
 */
(function(Q) {
	// 系统组件
	...
	/**
	 * websocket消息侦 first:byte
	 */
	function WsFrame(buffer) {
		var me = this, index = 0, b = buffer.readUInt8(index++), b = b & 255;
		me.fin = (b & 128) > 0; // boolean
		...
		if ((me.mask) == 0) { throw new Error("frame.notMasked"); }
		/**
		 * 传输数据的长度，以字节的形式表示：7位、7+16位、或者7+64位。 0-125这个范围，那这个值就表示传输数据的长度；
		...
		// 如果请求的是控制指令
		if (me.isControl()) {
			if (me.payloadLength > 125) { throw new Error("error opCode command,payloadLength is error"); }
			if (!me.fin) { throw new Error("error opCode command,fin is error"); }
		}
		/**
