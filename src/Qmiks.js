...
var Qmiks = new (function() {
	...
	function map(array, callback) {
		...
		each(array, function(i, v) {
			isfun ? r.push(execObject(callback, [
				i, v
			])) : r.push(v)
		});
		return r
	...
	function inArray(value, array) {
		if (likeArray(array)) for ( var i = 0; i < array.length; i++)
			if (array[i] === value) i;
		return -1
	}
	...
	/**
	 * 继承函数 可以有多个参数(参数都是对象,正常情况下是json类型的对象,不能是int等类型的基本类型变量) 1个参数时,表示对Q对象设置或替换新方法
	...
	Q.extend( {
		...
		getQmiksSrc : function() {// 取得当前qmiks框架源码所在目录
			return __dirname;
		}
	});
	function isQmiks(v) {
