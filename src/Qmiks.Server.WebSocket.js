...
(function(Q) {
	...
	function isSupportWebSocket(header) {
		return Header.UPGRADE.equalsIgnoreCase(header[Header.CONNECTION].toString()) && Header.WEBSOCKET
						.equalsIgnoreCase(header[Header.UPGRADE].toString());
	}
			...
			if (chr == 10 && tstart != i - 1) {
				...
				if (nb.length > 0) {
					if (lineNum == 1) {
						...
						var name = [], index = 0, isMove = false, tv;
	...
	function sendHandShake(header, socket) {
		...
		case 7:
			// 版本7
			var agentBuf = header.protocolAgent, host = header["Host"].toString(), location, index = 0, size = 0, varStr, buffer;
	...
	Q.inherit(WebSocket, net.Server);
	Q.extend(WebSocket.prototype, {
